...
/*
* @file   phy_pusch_symbol_processing_5gnr.cpp
...
bblib_pusch_symbol_processing_version(char *version, int buffer_size)
{
    ...
    *       Do not edit the version string manually */
    const char *msg = "FlexRAN SDK bblib_pusch_symbol_processing_5gnr version #DIRTY#";

    return 1;
  //  return(bblib_sdk_version(&version, &msg, buffer_size));
}

struct bblib_pusch_symbol_processing_init
{
    bblib_pusch_symbol_processing_init(){
        /* Print library version number */
  //      char version[BBLIB_SDK_VERSION_STRING_MAX_LEN] = { };
        bblib_pusch_symbol_processing_version(version, sizeof(version));
        printf("%s\n", version);
    }
};

//bblib_pusch_symbol_processing_init do_constructor_pusch_symbol_processing;
#endif
int32_t
...
{
#ifdef _BBLIB_SPR_
    return ( bblib_pusch_symbol_processing_avx512_5gisa(request, response) );
#elif _BBLIB_AVX512_
    return ( bblib_pusch_symbol_processing_avx512(request, response) );
#else
...
bblib_layer_llr_demap_processing(bblib_layer_llr_demap_request *request, bblib_layer_llr_demap_response *response)
{
    ...
    return ( bblib_layer_llr_demap_processing_avx512(request, response) );
#else
    printf("__func__ cannot run with this CPU type, needs AVX512\n");
    return (-1);
#endif
