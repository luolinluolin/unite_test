...
int main()
{
    ...
    request.nSymb = 12;
    request.nRxAnt = 4;
    request.nLayerInGroup = 1;
    request.nUeInGroup = 8;
    ...
    request.nEnable2ScProcess = 0; // 2SC processing
    request.nDMRSType = 1;
    request.nNrOfCDMs = 2;
    ...
    if (request.nLinInterpEnable == 1)
    {
        ...
        request.nGranularity = 4; // Linear3
    ...
    for (nOutLoop = 0; nOutLoop < OUT_LOOP; nOutLoop++){
        for (nInLoop = 0; nInLoop < IN_LOOP; nInLoop++){
            ...
            t1 = MLogTick();
	    uint16_t nProcRB = 6;
            request.nSubCarrier = nProcRB*12;
            for (int16_t iUe = 0; iUe < request.nUeInGroup; iUe++)
            {
	        int32_t nLlrOffset = 0;
	    	request.nStartSC = iUe * nProcRB;
                for (int j = 0; j < request.nSymb; j++) {
                    int16_t nSymb = request.pSymbIndex[j];
                    response.pLlr[nSymb][iUe] = pllrs[iUe] + nLlrOffset;
	            nLlrOffset += request.nSubCarrier * request.nLayerInGroup * request.eModOrder[iUe];
                }

#ifdef _BBLIB_SPR_
            bblib_pusch_symbol_processing_avx512_5gisa(&request, &response);
#else
            bblib_pusch_symbol_processing_avx512(&request, &response);
#endif
            }
