...
const static auto fp16Int16 = DataType<T>::fp16Int16;
    ...
    static void gain_sinr2(F32vec16 finvARe[N_TX][N_TX], F32vec16 ftempARe[N_TX][N_TX], F32vec16 finvAIm[N_TX][N_TX],
        ...
        for (size_t i = 0; i < N_TX; i++) {
            ...
            ftempGain2[i]  = _mm512_permutex2var_ps(ftempGain[i], use_2nd_half, ftempGain[i]);
            ftempGain[i] = _mm512_permutex2var_ps(ftempGain[i], use_1st_half, ftempGain[i]);
            ftempPostSINR2[i] = _mm512_permutex2var_ps(ftempPostSINR[i], use_2nd_half, ftempPostSINR[i]);
            ftempPostSINR[i] = _mm512_permutex2var_ps(ftempPostSINR[i], use_1st_half, ftempPostSINR[i]);
        }
    }
    ...
    static void gain_sinr(CF16vec16 finvARe[N_TX][N_TX], CF16vec16 ftempARe[N_TX][N_TX], CF16vec16 ftempGain[N_TX], CF16vec16 ftempPostSINR[N_TX],
        ...
        for (size_t i = 0; i < N_TX; i++) {
            ...
            if constexpr (POST_SINR_FLAG == 1) {
                ...
                fsumPostSINR[i] += _mm512_cvtph_ps(_mm256_castph_si256(postSinrOut.real()));
            }
    ...
    static void gain_sinr2(CF16vec16 finvARe[N_TX][N_TX], CF16vec16 ftempARe[N_TX][N_TX], CF16vec16 ftempGain[N_TX], CF16vec16 ftempGain2[N_TX], CF16vec16 ftempPostSINR[N_TX],
        ...
        for (size_t i = 0; i < N_TX; i++) {
            ...
            ftempGain2[i]  = _mm512_permutex2var_ph(ftempGain[i], use_2nd_half, ftempGain[i]);
            ftempGain[i] = _mm512_permutex2var_ph(ftempGain[i], use_1st_half, ftempGain[i]);
            ftempPostSINR2[i] = _mm512_permutex2var_ph(ftempPostSINR[i], use_2nd_half, ftempPostSINR[i]);
            ftempPostSINR[i] = _mm512_permutex2var_ph(ftempPostSINR[i], use_1st_half, ftempPostSINR[i]);
        }
            ...
            for (size_t nSCIdx = 0; nSCIdx < nSubCarrier; nSCIdx = nSCIdx + 32) {
                ...
                for (size_t j = 0; j < N_TX; j ++) {
                    ...
                    for (size_t i = 0; i < N_RX; i++) {
                        ...
                        chTmp0[i][j] = _mm512_permutex2var_ph(temp0, m512LoadGran2, temp1);
                        temp0 = *pChIn[1][j][i];
                        temp1 = *(pChIn[1][j][i] + 1);
                        chTmp1[i][j] = _mm512_permutex2var_ph(temp0, m512LoadGran2, temp1);
                        if constexpr (fo_flag == FO_E::enable){
                            chTmp1[i][j] = fmulconj(chTmp1[i][j], FoOffsetTable[iDmrsidx1][j]);
                        }
                    }
