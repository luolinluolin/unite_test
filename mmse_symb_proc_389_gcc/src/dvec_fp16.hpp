...
public:
  ...
  friend CF16vec16 reduce_add_half(CF16vec16 a) {
      a += _mm512_shuffle_f32x4(_mm512_castph_ps (a, a, _MM_PERM_CDAB); // 11,10,9,8, 15,14,13,12, 3,2,1,0, 7,6,5,4
...
// real + imag in vec
inline CF16vec16 addRealImag(CF16vec16 a) {
...
// real - imag in vec
inline CF16vec16 subRealImag(CF16vec16 a) {
  const auto k_order = _mm512_set_epi8(63, 62, 63, 62, 59, 58, 59, 58, 55, 54, 55, 54, 51, 50, 51,
     50, 47, 46, 47, 46, 43, 42, 43, 42, 39, 38, 39, 38, 35, 34, 35, 34, 31, 30, 31, 30, 27, 26, 27,
     26, 23, 22, 23, 22, 19, 18, 19, 18, 15, 14, 15, 14, 11, 10, 11, 10, 7, 6, 7, 6, 3, 2, 3, 2);
  auto temp = _mm512_shuffle_epi8(_mm512_castph_si512(a), k_order);
  auto re = _mm512_maskz_sub_ph (0x55555555, a, CF16vec16(temp));
  return CF16vec16(re);
}
...
inline F16vec32 addIQ(__m512 a) {
  const auto k_order = _mm512_set_epi8(63, 62, 63, 62, 59, 58, 59, 58, 55, 54, 55, 54, 51, 50, 51,
     50, 47, 46, 47, 46, 43, 42, 43, 42, 39, 38, 39, 38, 35, 34, 35, 34, 31, 30, 31, 30, 27, 26, 27,
     26, 23, 22, 23, 22, 19, 18, 19, 18, 15, 14, 15, 14, 11, 10, 11, 10, 7, 6, 7, 6, 3, 2, 3, 2);
  auto temp = _mm512_shuffle_epi8(_mm512_castps_si512(a), k_order);
  auto re = _mm512_maskz_add_ph (0x55555555, _mm512_castps_ph(a), _mm512_castsi512_ph(temp));
  return F16vec32(re);
}
#if 0
...
inline FORCE_INLINE Mask32 cmpgt(const F16vec32 &a, const F16vec32 &b){return _mm512_cmp_ph_mask(a, b, _CMP_GT_OS);}
