...
/* all fxp is deliberately designed, to directly get output llr fxp 8S(LLR_FXP)*/
    ...
    inline constexpr float16 rnn_fp16_scale_unit = (float16) (1.0 / (1 << RNN_FP16_SCALE));
#endif
...
void gainCalc (CF16vec16 gain[N_TX],
    ...
    for (size_t i = 0; i < N_TX; i++) {
        ...
        if constexpr (POST_SINR_FLAG == 1) {
            ...
            fsumPostSINR[i] += _mm512_cvtph_ps(_mm256_cvtph_epi16 (postSinrOut.real()));
        }
...
#pragma nounroll
        for (size_t j = 0; j < N_RX; j ++) {
            ...
            #pragma unroll(N_RX>>1)
            for (size_t k = 0; k < N_RX; k += 2) {
                T rnnRe0 = _mm512_castps_ph(_mm512_set1_ps(finvRnnC[k][j]));
                T rnnRe1 = _mm512_castps_ph(_mm512_set1_ps(finvRnnC[k+1][j]));
...
void HTransMulInvRnn<CF16vec16, float, 1, 1>(CF16vec16 ChIn[1][1], float finvRnnC[1][1],
    ...
    auto rnnRe = _mm512_castsi512_ph(_mm512_castps_si512 (_mm512_set1_ps(finvRnnC[0][0])));
    ...
    if(N_TX % 2){
        ...
        for (size_t i = 0; i < N_TX; i ++) {
            ...
            for (size_t j = 0; j < N_RX; j ++) {
                ...
                invMatrixHTansInvRnnRe2[i][j] = _mm512_permutex2var_ph(invMatrixHTansInvRnnRe[i][j], use_2nd_half, invMatrixHTansInvRnnRe[i][j]);
                invMatrixHTansInvRnnRe[i][j] = _mm512_permutex2var_ph(invMatrixHTansInvRnnRe[i][j], use_1st_half, invMatrixHTansInvRnnRe[i][j]);
            }
        }
    }
    else{
